---
title: Part II - Metrics That Suggest Centrality or Complexity of a Concept
author: Rees Morrison
date: '2022-01-26'
slug: []
categories:
  - Analysis
tags: ["Metrics Used for This Series of Posts and Analysis of Themes",]
draft: yes
---
...
[Part I](partI) of this series of five posts (working notes for myself) provides an overview of the Themes analyzed in this introductory treatment of centrality and complexity.  This post lays out the metrics that underlie the analysis. I present all 20 of them, with the text covering similar topics and order: first, why I gathered the metric, next the source of the metric, then any calculations that were made, and finally a plot showing the results for the four preceding metrics.  

<!--more-->

The table below lists each metric, its source and its (preliminary) classification regarding whether it suggests centrality, complexity or a n approximate balance between the two categories.  The right-most column shows the basis for which I sorted the 20 metrics into five groups.  The following pages present the metrics by groups of four.
...
PlotOED <- ggplot(oed, aes(x = reorder(FirstUse, FirstUse), y = OEDBand)) +
  ...
  labs(x = "Earliest Written Use", y = "Frequency Band", title = "Oxford English Dictionary Frequency Bands", caption = "Princeton Public Library access to OED Online\n https://www.oed.com/") +
  theme_tufte(ticks = FALSE) +
  coord_flip()
...
The same reasoning that led to metrics from Google's Trillion Word data base and the OED frequency bands supports the inclusion of rankings of Themes among the thousand most common words in English. A combination of three sources created the list used here.  One is [1000 Most Common Words](https://1000mostcommonwords.com/1000-most-common-english-words/), a second was [Wikipedia](https://en.wikipedia.org/wiki/Most_common_words_in_English), and the third came from [Smart Words](https://www.smart-words.org/500-most-commonly-used-english-words.html).  

If the Theme was expressed as a plural, such as "Soldiers", I determined the ranking of the singular form.  Any Theme that was not included in the composite list received a ranking of 1001. 

For each Theme, the plot below shows its rank on the compiled list.\
...
What people write about in books gives insight into what is important to their intellectual lives.  For Themes, the metrics suggests complexity more than centrality.
...
Here are the four preceding plots, whose metrics share an emphasis on Theme frequency.

```{r frequency, fig.width = 8, fig.height = 9}

wrap_plots(PlotSyn & theme_minimal(), PlotOED & theme_minimal(), PlotTop & theme_minimal(), PlotNgram & theme_minimal(), ncol = 2)
...
A metric of how common a Theme is on the internet could point us toward its centrality, because it is foremost in the minds of many writers, or its complexity, because more can be written about the concept than about a less-frequently used concept. 
...
PlotNorvig <- ggplot(norvig, aes(x = reorder(Theme, Trillion), y = Trillion)) +
  ...
  labs(x = NULL, y = "Total Counts on Google's Trillion Word Corpus from the Web", title = "Google Trillion Word Counts", caption = "From https://norvig.com/ngrams/") +
  theme_tufte(ticks = FALSE) +
  theme(axis.text.x = element_text(angle = 0)) +
  scale_y_continuous(breaks = seq(0, 2e9, 200e6), labels = scales::comma) +  
  coord_flip()
...
Another hypothesis for the relative centrality of a Theme holds that the more people have written a term on the internet, the more relevant it is to their lives: the more central.  Thus, the number of hits from a powerful search engine for a Theme crudely reflects its relative currency and salience.

For this metric, I enter a search into Google for each Theme.  When the results page returned, I recorded the number of results from the top left of my screen (e.g., for Thinking it says "About 992,000,000 results".  A later post will incorporate comparable search data results from Bing.
...
PlotSearch <- ggplot(google, aes(x = reorder(Theme, GoogleResults), y = GoogleResults)) +
  geom_point(size = 3, fill = "black", color = colorNames()[5]) +
  labs(x = NULL, y = "Search Results on Google", title = "Google Search Results") +
  theme_tufte(ticks = FALSE) +
  theme(axis.text.x = element_text(angle = 0)) + 
  scale_y_continuous(labels = scales::comma) +
  coord_flip()
...
[Google Trends](https://support.google.com/trends/answer/4365533?hl=en&ref_topic=6248052) provides access to a largely unfiltered sample of actual search requests made using Google. To make comparisons between terms easier Google Trends normalizes the search results to the time and location of a query by the following process:

Each data point is divided by the total searches of the geography and time range it represents to compare relative popularity. The resulting numbers are then scaled on a range of 0 to 100 based on a topic’s proportion to all searches on all topics.
...
PlotGTrend <- ggplot(trends, aes(x = reorder(Theme, GSearches), y = GSearches)) +
  ...
  labs(x = NULL, y = "Trend Avg.", title = "Google Search Trends", caption = "\nSource - https://trends.google.com/trends/?geo=US") +
  theme_tufte(ticks = FALSE) +
  theme(axis.text.x = element_text(angle = 0)) +
...
## SEO Hashtags WordTracker

Marketers who maintain an online presence want their site or advertisement to attract viewers and persuade them to click and buy.  Thus, among many Search Engine Optimization (SEO) techniques, they use hashtags and keywords to capture terms that will lure searches by search engines to display their company in the first page or so of results.  SEO is the effort to game this interchange.  Thus, a Theme term in prominent use online in the form of hashtags suggests centrality: what people want to buy, not anything heady like complexity.
...
PlotTracker <- ggplot(tracker, aes(x = reorder(Theme, VolIAAT), y = VolIAAT)) +
  ...
  guides(fill = "none") +
  labs(x = NULL, y = "Total", title = "Volume plus IAAT from WordTracker", caption = "\nSource - https://www.wordtracker.com/") +
  theme_tufte(ticks = FALSE) +
  scale_y_continuous(labels = scales::comma) +
  coord_flip()
...
Here are the four preceding plots, whose metrics share an emphasis on Themes on the internet.

```{r internet, fig.width = 8, fig.height = 9}

wrap_plots(PlotNorvig & theme_minimal(), PlotSearch & theme_minimal(), PlotGTrend & theme_minimal(), PlotTracker & theme_minimal(), ncol = 2)
...
People say something quotable because they are witty and pithy, or because they deliberately craft something memorable in speech or writing.  Quotations may derive from deep insights (complexity) or from a humorous twist on an everyday topic (centrality), but I lean toward the number of quotations using a Theme as an indicator more of complexity.
...
A [Kaggle dataset](https://www.kaggle.com/akmittal/quotes-dataset) includes a 22 MB dataset in JSON format of quotations.  From its 36,937 unique quotes, I recorded how many have each Theme term. Adding the two results together for each Theme generated this metric.
...
The plot below displays the total of the counts from the two sources.\  
...
Poems, by their nature of compression, allusion, meter and rhyme, suggest that if a Theme appears in a title of a poem, it pertains to a thoughtful attribute of the poem: complexity.

Three online databases of poems provided the data for this metric: [Poetry.com](https://www.poetry.com/psearch/), [Poetry Foundation](https://www.poetryfoundation.org/), and [Poetry.org](http://poetry.org/). I added together the results returned by searches on them.
...
PlotPoems <- ggplot(poems, aes(x = reorder(Theme, PoemTotal), y = PoemTotal)) +
  ...
  guides(fill = "none") +
  labs(x = NULL, y = "Total hits", title = "Poem Totals from Three Databases", subtitle = NULL, caption = "\nSources - https://www.poetry.com/psearch/\nhttps://www.poetryfoundation.org//nhttp://poetry.org/") +
  theme_tufte(ticks = FALSE) +
  scale_y_continuous(labels = scales::comma) +
  coord_flip()
...
Theme terms appear in figurative, idiomatic expressions.  For example, from the Theme of Clothes, "hung out to dry" refers literally to drying clothes on a line, but the expression conveys metaphorically  abandoning someone in a fraught situation.  For each Theme I compiled idiomatic expressions that use the term or an associated term (such as "shirt" for Clothes, or "barn burner" for Fire).  The terms came from more than 38 online sources. Expressions become commonplace because they point toward centrality.

Counting the number of unique expressions (combining slight variations on them) resulted in the `r sum(express$Sayings)`shown in the graphic below.
...
PlotRead <- ggplot(readable, aes(x = reorder(Theme, AvgGrade), y = AvgGrade)) +
  ...
  guides(fill = "none") +
  theme_tufte(ticks = FALSE) +
  labs(y = "US School Grade Level", x = NULL, title = "Readability", caption = "Average of Flesch- Kincaid, SMOG, and Coleman-Liau") +
  theme(axis.text.x = element_text(angle = 0)) +
...
Here are the four preceding plots, whose metrics share an emphasis on how frequently Themes appear on the Internet.

```{r language, fig.width = 8, fig.height = 9}

wrap_plots(PlotQuotes & theme_minimal(), PlotPoems & theme_minimal(), PlotExpress & theme_minimal(), PlotRead & theme_minimal(), ncol = 2)
...
The online site [WorldCat.org](https://www.worldcat.org/advancedsearch) lets users search Format "Book", Language "English" and specify Audience "Any Audience", Content "Any Content." Before running searches on Themes, I removed Dissertations (because I have a separate metric for them) and used singular versions of Theme terms, except Trains.  The results appear in this format: "Results 1-10 of about 95,810".\
...
The metrics collected are the number of titles found on WorldCat.org and the plot in the group below presents them.\
...
PlotBooks <- ggplot(books, aes(x = reorder(Theme, BkTitles), y = BkTitles)) +
  ...
  labs(x = NULL, y = "Search Results", title = "Book Titles", caption = "\nSource -- https://www.worldcat.org/advancedsearch") +
  theme_tufte(ticks = FALSE) +
  theme(axis.text.x = element_text(angle = 0)) +
...
As with the titles of books, the choice of a title for a song may be artistic or it may be commercial: what will attract a prospective buyer of the record. If the title caters to the pragmatic, money-making style, centrality reins; if the title derives from a less facile, more complicated source, it could point toward the complexity of the Theme.  On the whole, given the competitive pressures surrounding the efforts to produce hit songs, I favor centrality. Obviously, too, those who pick song titles do not want to duplicate an earlier title.

To obtain a metric for how often Theme terms have appeared in the titles of songs, I searched [The Music Lyrics Database](http://www.mldb.org/).  On the header you click "Search" and then pick "title."  On July 26, 2021 MLDB.org claimed "Lyrics: 236,453, Albums: 23,932, Artists: 11,115." Multiple pages of results show at the bottom of the page, but the highest number on the bottom right provides the data (I multiplied the total pages of results [less 1] by 30 and added the number of results on the final page).
...
PlotSongs <- ggplot(lyrics, aes(x = reorder(Theme, SongTitles), y = SongTitles)) +
  ...
  labs(x = NULL, y = "No. of Titles", title = "Titles of Songs", caption = "\nSource - Music Lyrics Database, http://www.mldb.org/") +
  theme_tufte(ticks = FALSE) +
  theme(axis.text.x = element_text(angle = 0)) +
  coord_flip()
...
In the same vein as song titles, the use of a Theme term in a movie title might have several motivations.  But metrics based on a huge number of movie titles suggest that centrality drives more of the decisions -- the title should resonate immediately with many prospective movie-goers.  To the degree that titles of movies derive from the title of the book (or play or musical) the movie adapts, we will see overlap with book titles (or play synopses, as per below).
...
To our hypotheses of centrality, complexity, or both, we can add synopses of plays.  Plays suggest more commonly an indicator of complexity as they deal with human psychology and emotions.  Here we deal not with titles, but with brief overviews of the plays.
...
PlotPlays <- ggplot(plays, aes(x = reorder(Theme, PlaySynopsis), y = PlaySynopsis)) +
  ...
  theme_tufte(ticks = FALSE) +
  labs(y = "Play Synopses Containing Theme", x = NULL, title = "Play Synopses", caption = "Source -- http://www.playdatabase.com/login.asp") +
  theme(axis.text.x = element_text(angle = 0)) +
  scale_y_continuous(labels = scales::comma) +
  coord_flip()
...
wrap_plots(PlotBooks & theme_minimal(), PlotSongs & theme_minimal(), PlotMovies & theme_minimal(), PlotPlays & theme_minimal(), ncol = 2)

```

## PhD Dissertations, Count of Instances

The title of the exhaustive research paper that becomes a dissertation, of a depth, coverage and quality that merits the award of a doctorate decree, surely suggests complexity.

For this metric, [EBSCO Open Dissertations](https://biblioboard.com/opendissertations/) provides searchable access from 320+ universities around the world to more than 1.4 million theses and dissertations. For each Theme term, EBSCO returned results in the form of "Search Results: 1 - 10 of 3,527".  I used the final figure.
...
The quartet below displays the findings.\
...
PlotPhDs <- ggplot(phds, aes(x = reorder(Theme, Dissertations), y = Dissertations)) +
 ...
 labs(x = NULL, y = "Dissertations", title = "EBSCO Open Dissertations", caption = "\nSource - https://biblioboardcom/opendissertations/") +
  theme_tufte(ticks = FALSE) +
  theme(axis.text.x = element_text(angle = 0)) +
  scale_y_continuous(labels = scales::comma) +
  coord_flip()
...
Government regulations and laws focus mostly on serious topics (by which I mean to compare their topics to the more emotional or psychological topics of songs and poems for example).  Thus, in classifying this metric, I lean toward complexity.

On the United States government's [website for the Code of Federal Regulations](https://www.ecfr.gov/search) you can query the contents of the entire 70,000+ pages of the CFR. I searched Find, Title	"All titles," and Date "current."  The search engine returns, for example, "Showing the most relevant 50 of 454 matching results."  The final figure became the metric for the Theme term.  The maximum values returned are 10,000 matching results.  Another iteration of research might look at Eurolex, an online compendium of European Union documents.
...
PlotCFR <- ggplot(cfr, aes(x = reorder(Theme, CFRRefs), y = CFRRefs)) +
  ...
  guides(fill = "none") +
  labs(x = NULL, y = "Search Results", title = "U.S. Code of Federal Regulations", caption = "\nSource - https://www.ecfr.gov/search?") +
  theme_tufte(ticks = FALSE) +
  theme(axis.text.x = element_text(angle = 0)) +
  scale_y_continuous(labels = scales::comma) +
  coord_flip()
...
For this metric, I searched the [Digital Library](https://digitallibrary.un.org/search?) maintained by the United Nations.  I chose all document types and added all the results that displayed on the left side window (it breaks the results down by several kinds of documents).
...
The metric lends itself to a scatter-plot, as below, sorted by increasing numbers of documents.

```{r unPlot, eval = TRUE}
PlotUN <- ggplot(un, aes(x = reorder(Theme, UNTotal), y = UNTotal)) +
  geom_point(shape = 21, size = 3, fill = "peachpuff2") +
  labs(x = NULL, y = "Search Results", title = "United Nations Documents", caption = "\nSource - https://digitallibrary.un.org/search?") +
  theme_tufte(ticks = FALSE) +
  theme(axis.text.x = element_text(angle = 0)) +
  scale_y_continuous(labels = scales::comma) +
  coord_flip()
...
As of this writing, Themes from Art had written about `r nrow(majors)` Subthemes. I categorized each Subtheme according to which of `r length(unique(majors$Major))` college major I thought would be most likely to address it. The table below lists the Themes, subthemes, major, and the blog post where they are discussed, all sorted alphabetically by major.  I regard this metric as weakly complex.
...
After all my work to figure out the ten most common majors, standardize their names, classify subthemes by those ten majors -- a process I did again after waiting a month, and then picking the consensus major -- I tested my process with two friends.  So many disputes arose, and different interpretations of the task, that I have concluded this metric as compiled is not reliable. To compound the shakiness of the foundational data by calculating a complexity score makes even less sense.  Nevertheless, I have left it in for this series as a record.\

```{r majorsPlotTwo, eval=TRUE}
PlotMajor <- ggplot(subTMajor, aes(x = Theme, y = Total, fill = Major)) +  
  geom_col() +
  # scale_fill_viridis() +
  labs(x = NULL , y = "Majors for the 24 Themes", title = "Majors of Subthemes",  caption = paste0(number_of_themes)) +
  theme_tufte(ticks = FALSE) +
...
```{r complexity, fig.width = 8, fig.height = 9}

wrap_plots(PlotPhDs & theme_minimal(), PlotCFR & theme_minimal(), PlotUN & theme_minimal(), PlotMajor & theme_minimal(), ncol = 2)

# library(cowplot)
# https://cran.r-project.org/web/packages/cowplot/vignettes/introduction.html  Read the beginning about themes chosen for ggplot.  See minimal horizontal and cowplot
# plot_grid(PlotExpress, PlotSyn, PlotRead, PlotUN, labels = c('', '', '', '')) # too small

# plot_annotation(tag_levels = "I") # Uppercase roman numerics

```

### Normalize the Metrics

Once all of the data described above had been collected for the 24 Themes, I created a single dataframe and converted the raw numbers of each metric into a normalized score.  This is an important step so that numbers which vary significantly in size, such as the frequency band digits of the Oxford English dictionary, which range from 0 to 8, can be compared to the number of Google search hits, which soar into the billions.  The base R function scale() subtracts the average of the metric (the mean) from each value and divides that result by the standard deviation of the metric. By that technique, the metrics (a.k.a. variables or features) are centered around zero and have roughly a variance of one (unit variance).

Another method of normalization converts each value into a score from 0 to 1, where the highest value would have the highest normalized score and the lowest would have the lowest normalized score -- but all of them would fall into the zero-to-one range. Another series may consider alternative methods of standardizing the metrics.\
